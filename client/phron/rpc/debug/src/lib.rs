	...
	C::Api: ApiExt<B>,
{
	...
	fn handle_params(params: Option<TraceParams>) -> RpcResult<(TracerInput, TraceType)> {
			...
			TraceType::CallList => {
				...
				let response = match tracer_input {
					TracerInput::CallTracer => {
						fc_evm_tracing::tracer::CallTracer::format(proxy)
			...
			if let Some(transaction) = transactions.get(index) {
				...
				return match trace_type {
					TraceType::Raw {
						...
						proxy.using(f)?;
						Ok(Response::Single(
							fc_evm_tracing::tracer::Raw::format(proxy).ok_or(internal_err(
